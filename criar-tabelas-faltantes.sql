...
-- 2. Criar tabela membros (se não existir)
...
-- 4. Criar tabela pipelines (se não existir)
...
-- 5. Criar tabela pipeline_etapas (se não existir)
...
-- 6. Criar tabela negocios (se não existir)
CREATE TABLE IF NOT EXISTS public.negocios (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    workspace_id UUID NOT NULL REFERENCES public.workspaces(id) ON DELETE CASCADE,
    cliente_id UUID REFERENCES public.clientes(id) ON DELETE SET NULL,
    pipeline_etapa_id UUID NOT NULL REFERENCES public.pipeline_etapas(id) ON DELETE RESTRICT,
...
-- 8. Aplicar trigger em todas as tabelas
...
CREATE INDEX IF NOT EXISTS idx_negocios_pipeline_etapa_id ON public.negocios(pipeline_etapa_id);

-- 12. Inserir dados iniciais
-- Workspace padrão
INSERT INTO public.workspaces (nome, slug, descricao) 
VALUES ('Nexus ERP', 'nexus-erp', 'Workspace padrão do sistema')
